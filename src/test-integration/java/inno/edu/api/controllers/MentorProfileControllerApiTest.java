...
public class MentorProfileControllerApiTest extends ApiTest {
    ...
    @Test
    public void shouldGetProfileByMentorId() throws Exception {
        this.mockMvc.perform(get("/api/mentor-profiles/mentor/" + feiProfile().getMentorId().toString())).andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id", is(feiProfile().getId().toString())))
                .andExpect(jsonPath("$.mentorId", is(feiProfile().getMentorId().toString())))
                .andExpect(jsonPath("$.universityId", is(feiProfile().getUniversityId().toString())))
